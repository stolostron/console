...
SA_SECRET=$(oc get pods -n open-cluster-management --selector=app=console-chart-v2,component=console -o json | jq -r '.items[0].spec.volumes[] | select(.name | test("-token-")).secret.secretName')

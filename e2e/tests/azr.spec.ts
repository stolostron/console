...
describe(`azr provider`, function () {
    it(`provider connection should not exist before create`, async function () {
        ...
        expect(
            providerConnections.find(
                (providerConnection) => providerConnection.metadata.name === 'azr-e2e-test-connection'
            )
        ).toBeFalsy()
    })
    ...
    //     expect(
    //         managedClusters.find((managedCluster) => managedCluster.metadata.name === 'azr-e2e-test-cluster')
    //     ).toBeFalsy()
    // })
    ...
    it(`provider connection should delete`, async function () {
        const result = await request.post(`cluster-management/graphql`, {
            query: /* GraphQL */ `
                mutation {
                    deleteProviderConnection(name: "azr-e2e-test-connection", namespace: "default")
                }
            `,
        })

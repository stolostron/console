...
function getRemoteArgoApps(ocpAppSetFilter: Set<string>, remoteArgoApps: IResource[]) {
  ...
  argoApps.forEach((argoApp: IArgoAppRemoteResource) => {
    // skip hub cluster apps since they're already handled by local watches
    // when the hub has a custom name, these apps can leak through the search filter
    ...
    if (argoApp._hostingResource) {
      ...
      if (kind === 'ApplicationSet') {
        ...
        // this app is generated by an applicationset, so keep it with the parent
        // and don't show it as a standalone app in the list
        return
      }
    }

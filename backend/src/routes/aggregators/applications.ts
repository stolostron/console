...
import { ApplicationSetKind, type IApplicationSet, type IResource } from '../../resources/resource'
import type { FilterSelections } from '../../lib/pagination'
...
export async function aggregateRemoteApplications(pass: number) {
  ...
  // //////////// SAVE RESULTS ///////////////////
  const argoAppSet = await cacheArgoApplications(
    applicationCache,
    (results.data?.searchResult?.[0]?.items ?? []) as IResource[]
  )
  await cacheOCPApplications(
    applicationCache,

...
import { getAppSetRelatedResources, inflateAppSetApps } from './applicationsArgo'
    ...
    const result: IUIData = {
      ...
      appSetRelatedResources:
        resource.kind === 'ApplicationSet'
          ...
          : ['', []],
      appSetApps: resource.kind === 'ApplicationSet' ? await inflateAppSetApps(resource.metadata.name) : [],
    }

...
describe('useFleetSearchPoll', () => {
  ...
  describe('data transformation', () => {
    ...
    it('should handle Pod resource transformation with conditions', () => {
      const podItemWithConditions = {
        ...mockSearchResultItem,
        kind: 'Pod',
      ...
      mockUseSearchResultItemsQuery.mockReturnValue({
        data: {
          searchResult: [{ items: [podItemWithConditions] }],
        },
        loading: false,
        error: undefined,
        refetch: jest.fn(),
      ...
      const watchOptionsPod = {
        ...mockWatchOptions,
        groupVersionKind: { group: '', version: 'v1', kind: 'Pod' },
      }

      const { result } = renderHook(() => useFleetSearchPoll(watchOptionsPod))
      const [data] = result.current
      const dataArray = data as any[]
      expect(data).toBeDefined()
      expect(dataArray[0].status.conditions).toEqual([
    ...
    it('should handle Search.search.open-cluster-management.io resource transformation', () => {
      const searchItemWithConditions = {
        ...mockSearchResultItem,
        kind: 'Search',
      ...
      mockUseSearchResultItemsQuery.mockReturnValue({
        data: {
          searchResult: [{ items: [searchItemWithConditions] }],
        },
        loading: false,
        error: undefined,
        refetch: jest.fn(),
      ...
      const watchOptionsSearch = {
        ...mockWatchOptions,
        groupVersionKind: { group: 'search.open-cluster-management.io', version: 'v1', kind: 'Search' },
      }

      const { result } = renderHook(() => useFleetSearchPoll(watchOptionsSearch))
      const [data] = result.current
      const dataArray = data as any[]
      expect(data).toBeDefined()
      expect(dataArray[0].status.conditions).toEqual([
    ...
    it('should handle MultiClusterHub.operator.open-cluster-management.io resource transformation', () => {
      const mchItemWithConditions = {
        ...mockSearchResultItem,
      ...
      mockUseSearchResultItemsQuery.mockReturnValue({
        data: {
          searchResult: [{ items: [mchItemWithConditions] }],
        },
        loading: false,
        error: undefined,
        refetch: jest.fn(),
      ...
      const watchOptionsMCH = {
        ...mockWatchOptions,
        groupVersionKind: { group: 'operator.open-cluster-management.io', version: 'v1', kind: 'MultiClusterHub' },
      }

      const { result } = renderHook(() => useFleetSearchPoll(watchOptionsMCH))
      const [data] = result.current
      const dataArray = data as any[]
      expect(data).toBeDefined()
      expect(dataArray[0].status.conditions).toEqual([

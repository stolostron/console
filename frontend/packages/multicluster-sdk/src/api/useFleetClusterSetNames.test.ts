...
import { useFleetClusterSetNames } from './useFleetClusterSetNames'
...
describe('useFleetClusterSetNames', () => {
  afterEach(() => {
    jest.clearAllMocks()
  })
  ...
  describe('when considerAllClusters is false (default)', () => {
    it('should return empty array when no clusters are loaded', () => {
      ...
      const { result } = renderHook(() => useFleetClusterSetNames())
    ...
    it('should return empty array when no clusters are loaded (explicit false)', () => {
      ...
      const { result } = renderHook(() => useFleetClusterSetNames(false))
    ...
    it('should return cluster set names only for clusters with cluster proxy addon available, ManagedClusterConditionAvailable: True, and clusterset label', () => {
      ...
      expect(result.current).toEqual([['clusterset-1', 'default'], true, undefined])
    })
  ...
  describe('when considerAllClusters is true', () => {
    it('should return all cluster set names regardless of labels and conditions', () => {
      ...
      const { result } = renderHook(() => useFleetClusterSetNames(true))
    ...
    it('should still filter out clusters without names or clusterset labels', () => {
      ...
      expect(result.current).toEqual([['clusterset-1', 'default', 'clusterset-3'], true, undefined])
    })
    ...
    it('should return default cluster set when no clusters have clusterset labels', () => {
      const mockClusters = [
        {
          metadata: {
            name: 'cluster-1',
      ...
      expect(result.current).toEqual([['default'], true, undefined])
    })

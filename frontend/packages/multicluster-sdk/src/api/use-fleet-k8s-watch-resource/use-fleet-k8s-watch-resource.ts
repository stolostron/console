...
const fleetSocketCache: Record<string, WSFactory> = {}
...
>(
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (fleetResourceCache[requestPath]) {
        return
      }
      ...
      try {
        ...
        if (!cachedSocket || cachedSocket?.getState() !== 'open') {
          const socket = fleetWatch(model, watchQuery)

          fleetSocketCache[socketKey] = socket

          socket.onbulkmessage((events) => {
            try {

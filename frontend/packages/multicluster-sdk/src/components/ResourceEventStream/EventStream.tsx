...
import { fleetWatch, useHubClusterName } from '../../api'
...
const ResourceEventStream: FC<{ resource: K8sResourceCommon }> = ({ resource }) => {
  ...
  useEffect(() => {
    if (!resource.cluster || resource.cluster === hubCluster) return
    ...
    if (!ws.current) {
      ws.current = fleetWatch(EventModel, watchURLOptions)

      if (ws.current === undefined) return

      ws.current.onbulkmessage((messages: { type: string; object: EventKind }[]) => {
        setSortedEvents((currentSortedEvents) => {
            ...
            (acc, { object, type: eventType }) => {
              ...
              switch (eventType) {
                ...
                case 'MODIFIED':
                  if (
                    acc?.[uid] &&
                    acc?.[uid]?.count !== undefined &&
                    object?.count !== undefined &&
                    acc?.[uid]?.count > object?.count
                  ) {
          ...
          return sortEvents(Object.values(batch))
        })
      })

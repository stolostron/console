...
export function ArgoWizard(props: ArgoWizardProps) {
    ...
    useEffect(() => {
        ...
        if (applicationSet) {
            const channel = gitChannels.find(
                (channel: any) => channel === applicationSet.spec.template.spec.source.repoURL
            )
            if (channel) {
                                ...
                                <WizAsyncSelect
                                    ...
                                    onValueChange={(value, item) => {
                                        const channel = props.channels?.find(
                                            (channel) =>
                                                channel?.spec?.pathname === item.spec.template.spec.source.repoURL
                                        )
                                        const path = createdChannels.find(
                                            (channel) => channel === item.spec.template.spec.source.repoURL
                                        )
                                        setGitPathsAsyncCallback(
                                            () => () =>
                                                getGitPathList(
...
function ArgoWizardPlacementSection(props: {
    ...
    const namespaceClusterSetNames =
        props.clusterSetBindings
            ?.filter(
                (clusterSetBinding) => clusterSetBinding.metadata?.namespace === applicationSet?.metadata?.namespace
            )

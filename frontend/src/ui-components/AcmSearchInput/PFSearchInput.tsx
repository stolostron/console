...
import { AdvancedSearchMenu } from '@patternfly/react-core/dist/js/components/SearchInput/AdvancedSearchMenu'
...
const SearchInputBase: React.FunctionComponent<SearchInputProps> = ({
  ...
  const searchInputExpandableToggleRef = React.useRef<HTMLElement | null>(null)
  ...
  React.useEffect(() => {
    setIsSearchMenuOpen(isAdvancedSearchOpen || false)
  }, [isAdvancedSearchOpen])
  ...
  const getAttrValueMap = () => {
    ...
    pairs.map((pair: string) => {
      ...
      if (splitPair.length === 2) {
        ...
        attrValue.haswords = Object.prototype.hasOwnProperty.call(attrValue, 'haswords')
  ...
  const onClearInput = (e: React.SyntheticEvent<HTMLButtonElement>) => {
    ...
    if (searchInputInputRef?.current) {
      searchInputInputRef.current.focus()
    }
  }
  ...
  const onExpandHandler = (event: React.SyntheticEvent<HTMLButtonElement>) => {
    setSearchValue('')
    if (onToggleExpand) {
      onToggleExpand(event, isExpanded || false)
    }
  ...
  const renderUtilities =
    value ||
    (enableUtilities && (resultsCount || (!!onNextClick && !!onPreviousClick) || (!!onClear && !expandableInput)))

...
describe('AcmExpandableSection', () => {
  ...
  test('Handles expanded prop change on rerender', () => {
    ...
    expect(container.querySelector('.pf-v6-c-expandable-section.pf-m-expanded')).toBeInTheDocument()
    rerender(
      <AcmExpandableSection expanded={false} label="Expandable Label" summary="Summary about this section">
        Section content
      </AcmExpandableSection>
    )
    expect(container.querySelector('.pf-v6-c-expandable-section.pf-m-expanded')).not.toBeInTheDocument()
  })
})

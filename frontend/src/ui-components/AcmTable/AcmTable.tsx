...
const expandableRowClass = 'pf-v5-c-table__expandable-row-content'
...
export function AcmTable<T>(props: AcmTableProps<T>) {
  ...
  const isKebabMenu = (cell: any): boolean => {
    return (
      typeof cell === 'object' &&
      'title' in cell &&
      typeof cell.title === 'object' &&
      'props' in cell.title &&
      cell.title.props?.children?.props?.isKebab === true
    )
  }
          ...
          <div ref={outerDivRef} className={outerDivClass}>
            <div ref={tableDivRef} className={tableDivClass}>
              ...
              >
                <Thead>
                  <Tr>
                    {onCollapse && <Th screenReaderText="Row collapse" />}
                    {hasSelectionColumn && <Th screenReaderText="Row select" />}
                    {selectedSortedCols
                  ...
                  return (
                    <Tbody isExpanded={row.isOpen} key={`${row.props.key}-tablebody`}>
                      <Tr key={`${row.props.key}-tablerow`} ouiaId={row?.props?.key}>
                        {onCollapse && (
                          <Td expand={{ isExpanded: row.isOpen || false, rowIndex, onToggle: onCollapse }}></Td>
                        )}
                        {hasSelectionColumn && (
                          <Td
                        ...
                        {row?.cells?.map((cell, cellIndex) => {
                          ...
                          const iTransformCellProps = transforms
                            ...
                            : []
                          const isMenuKebab = isKebabMenu(cell)
                          const rowProps = {
                            dataLabel: isMenuKebab ? undefined : columns[cellIndex].header,
                            ...iTransformCellProps,
                            className: isMenuKebab ? rowActionClass + ' ' + iTransformCellProps?.className : '',
                          }
                          return (
                            <Td key={`row-${rowIndex}-cell-${cellIndex}`} {...rowProps}>
                      ...
                      {addedSubRowCount > 0 && (
                        <Tr isExpanded={row.isOpen} key={addedSubRows[rowIndex]?.props?.key}>
                          ...
                          <Td key={addedSubRows[rowIndex]?.props?.key} colSpan={9}>

...
export function useValidation() {
  ...
  return useMemo(() => {
    const validateKubernetesDnsName = (value: string) => {
      if (value) {
        if (value.length > 63) return t('validate.kubernetesDnsName.length')
        for (const char of value) {
          if (char !== '-' && !lowercaseAlphaNumericCharacters.includes(char))
    ...
    const validatePublicSshKey = (value: string, required: boolean) => {
      if (!value && !required) {
        return undefined
      }
      if (value) {
    ...
    const validatePrivateSshKey = (value: string, requireNewline = true) => {
    ...
    const validateCertificate = (value: string) => {
    ...
    const validateGCProjectID = (value: string) => {
    ...
    const validateBaseDnsName = (value: string) => {
    ...
    const validateLibvirtURI = (value: string) => {
    ...
    const validateCloudsYaml = (yamlValue: string, cloudValue: string, osCABundle: string) => {
      if (yamlValue) {
        try {
    ...
    const validateAnsibleHost = (url: string, supportedProtocols?: string[]) => {
      const protocols = supportedProtocols ?? ['http', 'https']
      if (
        validator.isURL(url, {
    ...
    const validateAwsRegion = (value: string) => {
    ...
    const validateImageContentSources = (value: string) => {
      if (value) {
        try {
          //ensure we have valid YAML
    ...
    const validateHttpProxy = (value: string) => {
      if (value) {
        if (
    ...
    const validateHttpsProxy = (value: string) => {
      if (value) {
        if (
          validator.isURL(value, {
            require_protocol: true,
    ...
    const validateNoProxyList = (value: string) => {
      if (value) {
    ...
    const validateKubernetesResourceName = (value: string) => {
    ...
    const validatePolicyName = (value: string, resource: unknown) => {
    ...
    const validateAppSetName = (value: string, resource: unknown) => {
      const error = validateKubernetesResourceName(value)
      if (error) return error
    ...
    const validateVcenterUsername = (value: string) => {
    ...
    const validateKubeconfig = (value: string) => {
      if (value) {
        try {
    ...
    return {
      ...
      validateHttpProxy,
      validateHttpsProxy,
      validateHttpsURL: (value: string) => validateHttpsURL(value, t),
      validateNoProxy: (value: string) => validateNoProxy(value, t),
      validateNoProxyList,

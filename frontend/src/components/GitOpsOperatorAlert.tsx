...
export function GitOpsOperatorAlert(props: { showAlert: boolean; isPullModel: boolean; className?: string }) {
  ...
  const message = isPullModel
    ? t(
        'The OpenShift Gitops Operator is required on the hub cluster as well as all managed clusters you are targeting to create an applicaiton set pull model type.'
      )

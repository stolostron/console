...
function useClustersWithVirtualMachines() {
  ...
  const [virtualMachines, loaded, vmSearchError] = useFleetSearchPoll(
    {
      ...
      isList: true,
      limit: 1000, // Get enough results to identify all clusters
    },
  ...
  return {
    ...
    isLoading: !loaded, // Convert 'loaded' to 'isLoading'
    error: vmSearchError,
  }
}

export function KubevirtProviderAlert(props: {
  component: 'hint' | 'alert'
  className?: string
  /**
   * Indicates no hint is required when the operator is not installed.
   * Useful if a hint is already visible but the user is about to create a virtualization resource. */
  description?: string
  title?: string
  variant?: 'search' | 'clusterDetails'
  useLabelAlert?: boolean
  ...
  const FORCE_SHOW_UPGRADE_HUB_CLUSTER = true
  ...
  if (showInstallPrompt && !variant) {
    message = t('Install CNV to get more features.')
  }

  // Create action links based on hub version and variant
  const actionLinks = variant ? (
    <AcmActionGroup>
      {isHubVersionLessThan420 ? (
  ...
  if (props.useLabelAlert && !kubevirtOperator.pending && !isVMCountLoading && showInstallPrompt && variant) {
    const popoverActionLinks = (
      <div style={{ marginTop: '16px' }}>
        {isHubVersionLessThan420 || FORCE_SHOW_UPGRADE_HUB_CLUSTER ? (
          <Button
            variant="primary"
            onClick={() => {
              // TODO: Add upgrade logic or navigation
              window.open('/settings/cluster/', '_blank')
              console.log('Upgrade hub cluster clicked')
            }}
          ...
          <Button
            ...
            onClick={() => {
              window.open(multiClusterHubConsoleUrl ?? '', '_blank')
            }}
            style={{ marginRight: '8px' }}
          >
            {t('Edit MultiClusterHub')}

...
class YamlParser {
  ...
  parse(value, row) {
    ...
    while (this.moveToNextLine()) {
      ...
      isSequence = !!values
      if (!isSequence) {
        values = new RegExp('^(' + YamlInline.REGEX_QUOTED_STRING + '|[^ \'"[{].*?) *:(\\s+(.+?))?\\s*$').exec(
          this.currentLine
        )
        isMapping = !!values
      }
      if (isSequence) {
        ...
        if (
          ...
          if (
            ...
            ' ' == values.leadspaces &&
            (matches = new RegExp('^(' + YamlInline.REGEX_QUOTED_STRING + '|[^ \'"{[].*?) *:(\\s+(.+?))?\\s*$').exec(
              values.value
            ))
          ) {
            matches = {
        ...
        if (isProcessed) {
          ...
          if (this.isNextLineIndented() && !this.isNextLineUnIndentedCollection()) {
            if (data[key]) {
              throw new YamlParseException(`Duplicate key: ${key}`, this.getRealCurrentLineNb() + 1, this.currentLine)
            }
            obj.$v = null
            data[key] = obj
          } else {
  ...
  parseFoldedScalar(separator, indicator, indentation) {
    ...
    while (this.currentLineNb + 1 < this.lines.length) {
      ...
      let isSequence = false
      let isMapping = false
      matches = new RegExp('^( {' + textIndent.length + ',})(.+)$').exec(this.currentLineRaw)
      isSequence = !!matches
      if (!isSequence) {

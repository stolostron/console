...
export const addDiagramDetails = (resourceStatuses, resourceMap, isClusterGrouped, hasHelmReleases, topology) => {
  ...
  const eqIgnoreCase = (a) => (b) => String(a).toLowerCase() === String(b).toLowerCase()
  ...
  const clusterNamesList = _.sortBy(_.map(clustersObjects, 'name'))
  if (topology.nodes) {
    const appNode =
      _.find(
        topology.nodes,
  ...
  orderedList.forEach((kindArray) => {
    const relatedKindList = _.get(kindArray, 'items', [])

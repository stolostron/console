...
import { StatusModifier } from './style-utils'
import NodeLabel from './NodeLabel'
import NodeShadows, { NODE_SHADOW_FILTER_ID_DANGER, NODE_SHADOW_FILTER_ID_HOVER } from './NodeShadows'
import { DEFAULT_DECORATOR_RADIUS, getDefaultShapeDecoratorCenter, ShapeProps } from './shapeUtils'
...
const DefaultNode: React.FunctionComponent<DefaultNodeProps> = ({
  ...
  const status = nodeStatus || NodeStatus.success //element.getNodeStatus()
  ...
  const statusDecorator = React.useMemo(() => {
    ...
    return decorator
  }, [showStatusDecorator, status, getShapeDecoratorCenter, element, statusDecoratorTooltip])
  ...
  const ShapeComponent = (getCustomShape && getCustomShape(element)) || Ellipse
  ...
  const nodeLabelPosition = labelPosition || LabelPosition.bottom //element.getLabelPosition()

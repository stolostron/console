...
export function ApplicationOverviewPageContent(props: { name: string; namespace: string }) {
    ...
    const relatedSubscriptions = getAnnotation(selectedApplication[0], subscriptionAnnotationStr)
        ...
        for (let i = 0; i < subNamesList.length; i++) {
            ...
            if (!subscriptionName.endsWith('-local')) {
                const matchedSubscription = subscriptions.find(
                    (element) =>
                        element.metadata.name === subscriptionName &&
                        element.metadata.namespace === subscriptionNamespace
                )
        ...
        },
        {
            ...
            value: (
                <Fragment>
                    ...
                    {renderData(manualSyncTimestamp, createSyncButton(), '30%')}
    ...
    function createSubsCards(subsList: (Subscription | undefined)[], t: TFunction) {
        return (
            subsList.length &&
            subsList.map((sub) => {
                const appRepos = getApplicationRepos(selectedApplication[0], [sub] as Subscription[], channels)
                if (sub) {
                    return (

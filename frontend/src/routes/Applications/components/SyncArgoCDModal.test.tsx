...
describe('SyncArgoCDModal', () => {
  ...
  it('should render the modal for a single application', () => {
    ...
    expect(getByText('Sync ArgoCD application')).toBeTruthy()
    expect(getByText('Attempt to synchronize application resources with the source repository.')).toBeTruthy()
  ...
  it('should render the modal for an ApplicationSet', () => {
    ...
    expect(getByText('Sync ArgoCD application')).toBeTruthy()
    expect(
      getByText('Attempt to synchronize all applications in the ApplicationSet with their source repositories.')
    ).toBeTruthy()
  })
  ...
  it('should successfully sync a single application', async () => {
    ...
    await waitFor(() => {
      expect(patchResource).toHaveBeenCalled()
    })

    await waitFor(() => {
      expect(screen.getByText('ArgoCD app synced')).toBeInTheDocument()
    })
  ...
  it('should successfully sync an ApplicationSet', async () => {
    ...
    await waitFor(() => {
      // Should fetch latest version of both apps
      expect(getResource).toHaveBeenCalledTimes(2)
    })

    await waitFor(() => {
      // Should patch both apps

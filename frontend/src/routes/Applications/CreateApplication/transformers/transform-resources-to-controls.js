...
export const discoverGroupsFromSource = (control, cd, templateObject, editor) => {
    ...
    _.set(
        requeueTime,
        'active',
        _.get(applicationResource, 'spec.generators[0].clusterDecisionResource.requeueAfterSeconds')
    )
    // Template
    ...
    const decisionResourceControl = cd.find(({ id }) => id === 'decisionResourceName')
    ...
    // sync policy
    const syncOptions = _.get(applicationResource, 'spec.template.spec.syncPolicy.syncOptions', [])
    const automated = _.get(applicationResource, 'spec.template.spec.syncPolicy.automated', [])
    ...
    _.forEach(syncOptions, (option) => {
        switch (option) {
            ...
            case (option.match(/^PrunePropagationPolicy/) || {}).input: {
                ...
                control.active = true
                const propagationPolicy = cd.find(({ id }) => id === 'propagationPolicy')
                const propagationPolicySelection = option.slice(option.lastIndexOf('=') + 1)
...
const discoverChannelFromSource = (control, cd, templateObject) => {
    ...
    const source = _.get(templateObject, 'ApplicationSet[0].$raw.spec.template.spec.source')
    ...
    const channelType = _.get(control, 'controlData', {}).find(({ id: _id }) => _id === 'channelType')
    const resource = _.get(channelType, 'available', []).find(({ id: _id }) => _id === id)

    if (resource) {
        const insertControlData = _.get(resource, 'change.insertControlData', [])
        const channelNamespaceControl = insertControlData.find(({ id: _id }) => _id === 'channelNamespace')
        const channelNameControl = insertControlData.find(({ id: _id }) => _id === 'channelName')

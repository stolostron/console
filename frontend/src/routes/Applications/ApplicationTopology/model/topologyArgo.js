...
export const getSubscriptionPackageInfo = (
    topoAnnotation,
    subscriptionName,
    appNamespace,
    channelInfo,
...
export const createDeployableObject = (
    subscription,
    name,
    namespace,
    type,
...
export function buildArgoApplication(application, appName, appNamespace, nodes, links, cluster) {
    ...
    const clusterId = addClusters(
        ...
        null,
        clusterNames,
        _.uniqBy(clusters, 'metadata.name'),
        links,
        nodes
    )
...
async function getApplicationElements(application, clusterModel, cluster) {
    ...
    if (application.subscriptions) {
        ...
        application.subscriptions.forEach((subscription) => {
            ...
            const topoAnnotation = _.get(subscription, 'metadata.annotations', {})[
            ...
            if (
                _.get(subscription, 'spec.placement.local', '') === true &&
                subscription.rules &&

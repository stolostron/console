...
const discoveryVersions = ['4.22', '4.23', '5.0', '5.1', '5.2']
...
export function DiscoveryConfigPageContent(props: {
  ...
  const lastActiveArray: LastActive[] = useMemo(
    () =>
      [1, 2, 3, 7, 14, 21, 30].map((count) => ({
        day: count,
          ...
          <AcmSelect
            ...
            onChange={(lastActive) => {
              updateDiscoveryConfig((discoveryConfig) => {
                if (lastActive) {
                  discoveryConfig.spec.filters ??= {}
                  discoveryConfig.spec.filters.lastActive = Number.parseInt(

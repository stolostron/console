      ...
      <>
        <span style={{ whiteSpace: 'nowrap' }}>
          <Link to={getClusterNavPath(NavigationPath.clusterDetails, cluster)}>
            <HighlightSearchText text={cluster.displayName} searchText={search} isTruncate useTruncatePopover />
          </Link>
        </span>
...
export function useClusterNamespaceColumn(): IAcmTableColumn<Cluster> {
  ...
  return {
    ...
    cell: (cluster, search) => (
      <span style={{ whiteSpace: 'nowrap' }}>
        <HighlightSearchText text={cluster.namespace ?? '-'} searchText={search} isTruncate useTruncatePopover />
      </span>
    ),

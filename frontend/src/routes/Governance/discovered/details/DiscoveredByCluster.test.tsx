...
describe('DiscoveredByCluster', () => {
  ...
  test(
    'Should render DiscoveredByCluster for OperatorPolicy',
    async () => {
      const context: DiscoveredDetailsContext = {
        isFetching: false,
        relatedResources: [],
  ...
  test('Should render labels column with user-defined labels', async () => {
    ...
    // Labels are rendered in compact mode (popover) for consistency with DiscoveredPolicies table
    // Both clusters have 2 user-defined labels (env, team) - system labels filtered out
    // The popover shows "2 labels" as the trigger with unique IDs per row
    await waitForText('2 labels', true) // Multiple rows may have "2 labels"

    // Verify system labels are NOT shown in popover triggers
  ...
  test('Should have label filter options available', async () => {
    const context: DiscoveredDetailsContext = {
      isFetching: false,
      relatedResources: [],
      policyItems: [
    ...
    // Verify the label filter component is present (secondary filter with searchable select)
    const labelFilter = screen.getByTestId('acm-table-filter-select-Label')
    expect(labelFilter).toBeInTheDocument()

    // Test that the label filter logic works correctly using the matchesSelectedLabels predicate

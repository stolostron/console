...
import { clickByText, clickElement, waitForNocks, waitForNotText, waitForText } from '../../../lib/test-util'
...
describe('Create Policy Automation Wizard', () => {
  ...
  test(
    ...
    async () => {
      ...
      await clickElement(screen.getByPlaceholderText('Select the Ansible credential'))
      ...
      await clickElement(screen.getByPlaceholderText(/select the schedule/i))
      await clickElement(screen.getByRole('option', { name: 'Once' }))
  ...
  test('can cancel policy automation creation', async () => {

...
export function PolicyTemplateDetails() {
  ...
  }, [apiGroup, clusterName, template, paramKind.paramKindObj, kind, templateLoading])
  ...
  const descriptionItems = useMemo(() => {
    ...
    if (apiGroup === 'constraints.gatekeeper.sh') {
      ...
      if (vapb.loading && !vapb.vapbObj) {
        cols.push({
          key: '',
          keyAction: (
            <Skeleton width="100px" height="60px" screenreaderText="Fetching ValidatingAdmissionPolicyBinding" />
          ),
          value: <Skeleton width="100%" screenreaderText="Fetching ValidatingAdmissionPolicyBinding" />,
        })
      } else if (vapb.vapbObj && !vapb.loading) {
        cols.push({
    ...
    if (kind === 'ValidatingAdmissionPolicyBinding') {
      ...
      if (!template) {
        ...
        if (paramKind.loading) {
          cols.push({
            key: '',
            keyAction: <Skeleton width="100px" height="60px" screenreaderText="Fetching paramRef resource" />,
            value: <Skeleton width="100%" screenreaderText="Fetching paramRef resource" />,
          })
        } else if (template?.spec?.paramRef && paramKind.paramKindObj && !paramKind.loading) {

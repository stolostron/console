...
describe('RoleAssignmentActionDropdown', () => {
  ...
  it('opens dropdown when toggle is clicked', () => {
    ...
    expect(screen.getByText('Delete role assignment')).toBeInTheDocument()
  })
  ...
  it('calls setModalProps when delete option is clicked', () => {
    ...
    // Click delete option
    const deleteOption = screen.getByText('Delete role assignment')
  ...
  it('modal props contain correct role assignment data', () => {
    ...
    expect(modalProps.columns[0].header).toBe('Name')
    expect(modalProps.columns[1].header).toBe('Role')

    // Test column cell functions
    expect(modalProps.columns[0].cell(mockRoleAssignment)).toBe('test-role-assignment')
    expect(modalProps.columns[1].cell(mockRoleAssignment)).toBe('admin, viewer')
  })

...
import { listUsers, listClusterRoleBindings, ClusterRoleBinding } from '../../../../resources/rbac'
...
const UserRoleAssignments = () => {
  ...
  const { data: clusterRoleBindings, loading: rolesLoading } = useQuery(listClusterRoleBindings)
  ...
  const userRoleBindings = useMemo(() => {
    ...
    return clusterRoleBindings
      .filter((crb) => crb.subjects?.some((subject) => subject.kind === 'User' && subject.name === user.metadata.name))
  ...
  const filters = useMemo<ITableFilter<ClusterRoleBinding>[]>(() => {
    ...
    const roleNames = userRoleBindings.map((crb) => crb.roleRef.name)
    const uniqueRoleNames = Array.from(new Set(roleNames))
    const roleOptions = uniqueRoleNames.sort().map((role) => ({ label: role, value: role }))

    // Get unique kind options
    const kindNames = userRoleBindings.map((crb) => crb.roleRef.kind)
    const uniqueKindNames = Array.from(new Set(kindNames))
    const kindOptions = uniqueKindNames.sort().map((kind) => ({ label: kind, value: kind }))
    ...
    return [
      {
        ...
        label: t('Role'),
        options: roleOptions,
        tableFilterFn: (selectedValues, roleBinding) => selectedValues.includes(roleBinding.roleRef.name),
      },
      {
        id: 'kind',
        label: t('Kind'),
        options: kindOptions,
        tableFilterFn: (selectedValues, roleBinding) => selectedValues.includes(roleBinding.roleRef.kind),
      },
      {
        id: 'namespace',
        label: t('Scope'),
        ...
        tableFilterFn: (selectedValues, roleBinding) => {
          ...
          return isClusterWide ? selectedValues.includes('cluster-wide') : selectedValues.includes('namespaced')
        },
      },
    ]
  }, [userRoleBindings, t])

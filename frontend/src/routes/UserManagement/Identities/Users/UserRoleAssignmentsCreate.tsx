      ...
      {props.showYaml ? (
        <Drawer isExpanded={props.showYaml} isInline={true} style={{ height: '500px' }}>
          <DrawerContent
            style={{ height: '500px' }}
            panelContent={
              <DrawerPanelContent isResizable={true} defaultSize="50%" minSize="400px" style={{ height: '500px' }}>
            ...
            <DrawerContentBody style={{ padding: '24px', height: '500px', overflow: 'visible' }}>
              <Form>
                ...
                {scope === 'specific' && selectedClusters.length > 0 && (
                  ...
                  >
                    {props
                      .getNamespaces(props.clusters.filter((c) => selectedClusters.includes(c.metadata.name!)))
                      .map((namespace) => (
  ...
  return (
    <>
      <AcmModal
        ...
        hasNoBodyWrapper
        actions={[
          <AcmButton key="save" variant="primary" onClick={handleSaveClick} isDisabled={!canSave}>
            {t('Save')}
          </AcmButton>,

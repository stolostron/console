...
describe('BattchUpgradeModal', () => {
    ...
    it('should show loading when click upgrade, and upgrade button should be disabled when loading', async () => {
        ...
        await act(async () => {
            ...
            await waitFor(() => expect(mockNockUpgrade2.isDone()).toBeTruthy())
            await waitFor(() => expect(queryByText('upgrade.submit.processing')).toBeFalsy())
            await waitFor(() => expect(queryByText('upgrade.submit')).toBeTruthy())
            await waitFor(() => expect(isClosed).toBe(true))
        })
    })
    ...
    it('should show alert when failed; keep failed rows in table, and succeeded rows should show upgrading', async () => {
        ...
        await waitFor(() => expect(queryByText('upgrade.submit.processing')).toBeFalsy())
        await waitFor(() => expect(queryByText('upgrade.submit')).toBeTruthy())
        await waitFor(() => expect(queryByText('upgrade.multiple.upgradefailed')).toBeTruthy())
        expect(getByText('upgrade.istriggered')).toBeTruthy()
        expect(queryByText('2.2.6')).toBeTruthy()
        expect(queryByText('1.2.9')).toBeFalsy()
    })
})

...
export function DownloadConfigurationDropdown(props:{
    ...
    if (cluster?.hiveSecrets) {
        ...
        return (
            <AcmDropdown isDisabled={props.accessRestriction} tooltip={props.accessRestriction ? t('common:rbac.unauthorized') : undefined} dropdownItems={dropdownItems} onSelect={(id: string) => downloadConfig(id)} text={t('configuration.download')} id='download-configuration' />
        )

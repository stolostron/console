...
export function HiveNotification() {
    ...
    const provisionStatuses: string[] = [ClusterStatus.creating, ClusterStatus.destroying, ClusterStatus.provisionfailed, ClusterStatus.deprovisionfailed]
    ...
    return (
        <div style={{ marginBottom: '1rem' }} id={`hive-notification-${cluster?.status}`}>
            <AcmAlert
                isInline
                variant={(cluster?.status === ClusterStatus.provisionfailed || cluster?.status === ClusterStatus.deprovisionfailed) ? AlertVariant.danger : AlertVariant.info}
                title={
                    <Fragment>

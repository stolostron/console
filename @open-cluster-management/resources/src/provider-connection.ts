...
export function unpackProviderConnection(
    secret: ProviderConnection | V1Secret
) {
    ...
    if (providerConnection.data) {
        ...
        const stringData = providerConnection.stringData as Record<
            string,
            string
        >;
        for (const key in providerConnection.data) {
            stringData[key] = Buffer.from(data[key], "base64").toString(
                "ascii"
            );
        }
...
export function packProviderConnection(providerConnection: ProviderConnection) {
    ...
    if (stringData !== undefined) {
        for (const key in stringData) {
            data[key] = Buffer.from(stringData[key], "ascii").toString(
                "base64"
            );
        }

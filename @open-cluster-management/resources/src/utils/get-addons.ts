...
} from "../cluster-management-add-on";
...
export const addonTextKey =
    "console.open-cluster-management.io/launch-link-text";
...
export function mapAddons(
    ...
    const addons: Addon[] = managedClusterAddons.map((mca) => {
        let cma: ClusterManagementAddOn | undefined =
            clusterManagementAddons.find(
                (clusterManagementAddOn) =>
                    mca.metadata.name === clusterManagementAddOn?.metadata.name
            );
        if (cma === undefined) {
            cma = {
                ...ClusterManagementAddOnDefinition,
...
function getDisplayStatus(
    cma: ClusterManagementAddOn | undefined,
    mcas: ManagedClusterAddOn[]
): string {
    const mcaStatus = mcas?.find(
        (mca) => mca.metadata.name === cma?.metadata.name
    );
    if (mcaStatus?.status?.conditions === undefined) {
        return AddonStatus.Disabled;
    }
    const managedClusterAddOnConditionDegraded =
        mcaStatus?.status.conditions.find(
    ...
    const managedClusterAddOnConditionProgressing =
        mcaStatus?.status.conditions.find(
    ...
    const managedClusterAddOnConditionAvailable =
        mcaStatus?.status.conditions.find(
...
export function getLaunchLink(
    ...
    const cmaHasLaunchLink =
        cmaAnnotations.includes(addonPathKey) &&
        cmaAnnotations.includes(addonTextKey);
    if (mca) {
        const mcaAnnotations = Object.keys(mca.metadata.annotations ?? []);
        const mcaHasLaunchLink =
            mcaAnnotations.includes(addonPathKey) &&
            mcaAnnotations.includes(addonTextKey);
        if (mcaHasLaunchLink) {
            return {
            ...
            if (cmaHasLaunchLink) {
                return {
                    displayText:
                        cma?.metadata?.annotations?.[addonTextKey] ?? "",

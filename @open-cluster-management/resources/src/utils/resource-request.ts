...
const ResourceErrorCodes = Object.keys(ResourceErrorCode).map((k) =>
    Number(ResourceErrorCode[k as any])
);
...
export function createResource<
    ...
    ResultType = Resource
>(resource: Resource): IRequestResult<ResultType> {
...
export function patchResource<
    Resource extends IResource,
    ResultType = Resource
>(resource: Resource, data: unknown): IRequestResult<ResultType> {
    const url = backendUrl + getResourceNameApiPath(resource);
...
export function getResource<Resource extends IResource>(
    ...
    if (options?.labelSelector) {
        ...
        for (const key in options.labelSelector) {
            const value =
                options.labelSelector[key] !== undefined
    ...
    if (options?.fieldSelector) {
        ...
        for (const key in options.fieldSelector) {
            const value =
                options.fieldSelector[key] !== undefined
                    ? options.fieldSelector[key]
                    : "";
        ...
        queryString +=
            "fieldSelector=" + fields.map((field) => field).join(",");
    }
...
export function getRequest<ResultT>(url: string): IRequestResult<ResultT> {
    ...
    return {
        promise: fetchGet<ResultT>(url, abortController.signal).then(
            (result) => result.data
        ),
...
export function putRequest<DataT, ResultT>(
    ...
    const abortController = new AbortController();
    return {
        promise: fetchPut<ResultT>(url, data, abortController.signal).then(
            (result) => result.data
        ),
        abort: () => abortController.abort(),
    };
...
export function postRequest<DataT, ResultT>(
    ...
    const abortController = new AbortController();
    return {
        promise: fetchPost<ResultT>(url, data, abortController.signal).then(
            (result) => result.data
        ),
        abort: () => abortController.abort(),
    };
...
export async function fetchRetry<T>(options: {
    ...
    let retries =
        ...
        options.retries >= 0
            ...
            : 0;
    let delay =
        options?.delay && Number.isInteger(options.delay) && options.delay > 0
            ? options.delay
            : 100;
    ...
    if (options.data) {
        try {
            ...
            throw new ResourceError(
                `Invalid body object for request`,
                ResourceErrorCode.BadRequest
            );
        }
    }
        ...
        if (response) {
            ...
            if ((responseData as any)?.kind === StatusKind) {
                ...
                if (status.status !== "Success") {
                    if (status.code === 401) {
                        ...
                        ResourceErrorCodes.includes(status.code as number)
                    ) {
                        throw new ResourceError(
                            status.message as string,
                            status.code as number
            ...
            switch (response.status) {
                ...
                case 401: // 401 is returned from the backend if no token cookie is on request
                    ...
                    throw new ResourceError(
                        "Unauthorized",
                        ResourceErrorCode.Unauthorized
                    );

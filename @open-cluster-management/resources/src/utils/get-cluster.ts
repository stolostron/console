...
export function mapClusters(
    ...
    return uniqueClusterNames.map((cluster) => {
        const clusterDeployment = clusterDeployments?.find(
            (cd) => cd.metadata?.name === cluster
        );
        const managedClusterInfo = managedClusterInfos?.find(
            (mc) => mc.metadata?.name === cluster
        );
        const managedCluster = managedClusters?.find(
            (mc) => mc.metadata?.name === cluster
        );
        const addons = managedClusterAddOns.filter(
            (mca) => mca.metadata.namespace === cluster
        );
        const clusterClaim = clusterClaims.find(
            (clusterClaim) => clusterClaim.spec?.namespace === cluster
        );
        const clusterCurator = clusterCurators.find(
            (cc) => cc.metadata.namespace === cluster
        );
        return getCluster(
            managedClusterInfo,
            clusterDeployment,
...
export function getCluster(
    ...
    return {
        ...
        labels:
            ...
            managedClusterInfo?.metadata.labels,
...
const checkForCondition = (
    ...
    conditions?.find((c) => c.type === condition)?.status ===
    (status ?? "True");
...
export const getCuratorConditionMessage = (
    condition: string,
    conditions: V1CustomResourceDefinitionCondition[]
) => {
    const cond = conditions?.find((c) => c.type === condition);
...
export const checkCuratorConditionFailed = (
    condition: string,
    conditions: V1CustomResourceDefinitionCondition[]
) => {
    const cond = conditions?.find((c) => c.type === condition);
...
export const checkCuratorConditionDone = (
    condition: string,
    conditions: V1CustomResourceDefinitionCondition[]
) => {
    const cond = conditions?.find((c) => c.type === condition);
...
export const getConditionStatusMessage = (
    condition: string,
    conditions: V1CustomResourceDefinitionCondition[]
) => {
    const cond = conditions?.find((c) => c.type === condition);
...
export function getOwner(
    ...
    const cdUserIdentity =
        clusterDeployment?.metadata.annotations?.[userIdentity];
...
export function getHiveConfig(
    ...
    const supportsHibernation =
        hibernatingCondition?.status === "False" &&
        hibernatingCondition?.reason !== "Unsupported";
    ...
    return {
        ...
        clusterPoolNamespace:
            clusterDeployment?.spec?.clusterPoolRef?.namespace,
        clusterClaimName: clusterDeployment?.spec?.clusterPoolRef?.claimName,
        secrets: {
            kubeconfig:
                clusterDeployment?.spec?.clusterMetadata
                    ?.adminKubeconfigSecretRef.name,
            kubeadmin:
                clusterDeployment?.spec?.clusterMetadata?.adminPasswordSecretRef
                    .name,
            installConfig:
                clusterDeployment?.spec?.provisioning?.installConfigSecretRef
                    ?.name,
        },
...
export function getProvider(
    ...
    const hivePlatformLabel =
        clusterDeployment?.metadata?.labels?.[
            "hive.openshift.io/cluster-platform"
        ];
...
export function getDistributionInfo(
    ...
    if (managedCluster) {
        ...
        const versionClaim = managedCluster.status?.clusterClaims?.find(
            (cc) => cc.name === "version.openshift.io"
        );
    ...
    const productClaim: string | undefined =
        managedCluster?.status?.clusterClaims?.find(
    ...
    if (clusterCurator || managedClusterInfo) {
        ...
        upgradeInfo.latestJob.step =
            ...
            checkCuratorLatestOperation(
                ...
                ? CuratorCondition.posthook
                : CuratorCondition.prehook;
        const curatorIsIdle = !checkCuratorConditionInProgress(
            "clustercurator-job",
            curatorConditions
        );
        ...
        const isSelectingChannel =
            ...
            clusterCurator?.spec?.upgrade?.channel !==
                managedClusterInfo?.status?.distributionInfo?.ocp.channel &&
            !curatorIsIdle;

        const upgradeDetailedMessage =
            getCuratorConditionMessage("monitor-upgrade", curatorConditions) ||
            "";
        const percentageMatch = upgradeDetailedMessage.match(/\d+%/) || [];
        upgradeInfo.upgradePercentage =
            percentageMatch.length > 0 ? percentageMatch[0] : "";
        const desiredVersion =
            managedClusterInfo?.status?.distributionInfo?.ocp?.desired
                ?.version ||
        ...
        upgradeInfo.upgradeFailed =
            (!!desiredVersion &&
                desiredVersion !==
                    managedClusterInfo?.status?.distributionInfo?.ocp
                        ?.version &&
                managedClusterInfo?.status?.distributionInfo?.ocp
                    ?.upgradeFailed) ??
            false;
        ...
        upgradeInfo.posthooks = {
            hasHooks:
                (clusterCurator?.spec?.upgrade?.posthook ?? []).length > 0,
            inProgress:
                isUpgradeCuration &&
                checkCuratorConditionInProgress(
                    "posthook-ansiblejob",
...
export function getNodes(managedClusterInfo?: ManagedClusterInfo) {
    ...
    nodeList.forEach((node: NodeInfo) => {
        const readyCondition = node.conditions?.find(
...
export function getClusterStatus(
    ...
    if (clusterCurator) {
        const ccConditions: V1CustomResourceDefinitionCondition[] =
            clusterCurator.status?.conditions ?? [];
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    statusMessage = getConditionStatusMessage(
                        CuratorCondition.prehook,
                        ccConditions
                    );
                } else {
                ...
                !checkCuratorConditionDone(
                    CuratorCondition.monitor,
                    ccConditions
                )
            ) {
                ccStatus = checkCuratorConditionFailed(
                    CuratorCondition.monitor,
                    ccConditions
                )
                    ? ClusterStatus.provisionfailed
                    : checkCuratorConditionFailed(
                          CuratorCondition.provision,
                          ccConditions
                      )
                    ? ClusterStatus.provisionfailed
    ...
    if (clusterDeployment) {
        const cdConditions: V1CustomResourceDefinitionCondition[] =
            clusterDeployment?.status?.conditions ?? [];
        //const hasInvalidImageSet = checkForCondition('ClusterImageSetNotFound', cdConditions)
        const hasInvalidImageSet =
            checkForRequirementsMetConditionFailureReason(
                "ClusterImageSetNotFound",
                cdConditions
            );
        const provisionFailed = checkForCondition(
            "ProvisionFailed",
            cdConditions
        );
        const provisionLaunchError = checkForCondition(
            "InstallLaunchError",
            cdConditions
        );
        const deprovisionLaunchError = checkForCondition(
        ...
        if (deprovisionLaunchError) {
            ...
            const hibernatingCondition =
                clusterDeployment?.status?.conditions?.find(
                    (c) => c.type === "Hibernating"
                );
            ...
            if (hasInvalidImageSet) {
                const invalidImageSetCondition = cdConditions.find(
                    (c) =>
                        c.type === "RequirementsMet" &&
                        c.reason === "ClusterImageSetNotFound"
                );
                cdStatus = ClusterStatus.provisionfailed;
                statusMessage = invalidImageSetCondition?.message;
            } else if (provisionFailed) {
                const provisionFailedCondition = cdConditions.find(
                    (c) => c.type === "ProvisionFailed"
                );
                const currentProvisionRef =
                    clusterDeployment.status?.provisionRef?.name ?? "";
                if (
                    provisionFailedCondition?.message?.includes(
                        currentProvisionRef
                    )
                ) {
                    cdStatus = ClusterStatus.provisionfailed;
    ...
    const mcConditions: V1CustomResourceDefinitionCondition[] =
        mc.status?.conditions ?? [];
    const clusterAccepted = checkForCondition(
        "HubAcceptedManagedCluster",
        mcConditions
    );
    const clusterJoined = checkForCondition(
        "ManagedClusterJoined",
        mcConditions
    );
    const clusterAvailable = checkForCondition(
    ...
    if (mc?.metadata.deletionTimestamp) {
        ...
        if (certificateSigningRequests && certificateSigningRequests.length) {
            const clusterCsrs =
                certificateSigningRequests?.filter((csr) => {
                    return (
                        csr.metadata.labels?.[CSR_CLUSTER_LABEL] ===
                        mc.metadata.name
                    );
                }) ?? [];
            const activeCsr = getLatest<CertificateSigningRequest>(
                clusterCsrs,
                "metadata.creationTimestamp"
            );
            mcStatus =
                activeCsr && !activeCsr?.status?.certificate
                    ? ClusterStatus.needsapproval
        ...
        if (clusterAvailable) {
            ...
            mcStatus = hasDegradedAddons
                ? ClusterStatus.degraded
                : ClusterStatus.ready;
        } else {
            const clusterUnavailable = checkForCondition(
                "ManagedClusterConditionAvailable",
                mcConditions,
                "False"
            );
